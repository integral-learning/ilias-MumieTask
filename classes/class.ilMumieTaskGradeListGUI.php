            ...
            if (!empty($xGrades)) {
                foreach ($xGrades as $xGrade) {
                    if ($xGrade->actor->account->name == $syncId) {
                        ...
                        $this->tpl->setVariable("VAL_TIME", substr($xGrade->timestamp, 11, strlen($xGrade->timestamp)-5));
                        ilLoggerFactory::getLogger('xmum')->info(substr($xGrade->timestamp, 11, strlen($xGrade->timestamp)-5));
                        $this->tpl->setCurrentBlock("tbl_content");
