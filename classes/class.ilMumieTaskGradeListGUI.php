            ...
            if (!empty($xGrades)) {
                foreach ($xGrades as $xGrade) {
                    if ($xGrade->actor->account->name == $syncId) {
                        ...
                        $this->tpl->setVariable("VAL_TIME", substr($xGrade->timestamp, 11, strlen($xGrade->timestamp)-2));
                        ilLoggerFactory::getLogger('xmum')->info(substr($xGrade->timestamp, 11, strlen($xGrade->timestamp)-2));
                        $this->tpl->setCurrentBlock("tbl_content");
